...
This does not complete `scripts/generate-changelog.ts`: no ledger interval is
marked mapped by this work. JavaScript date parsing of malformed or overflowing
calendar dates, the complete original feed test corpus, site pipeline integration,
and release publication verification remain open. The source is MIT-licensed by
...
- Pinned main source `feed` test group: seven tests / 14 assertions passed in the
  ...
  seven original tests have already received individual Rust ledger mappings.
