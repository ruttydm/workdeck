...
const TEST_STABLE_SHA256: &str =
    "f89381df3304faff020ef1b489d32d7e52fe437dafad9bcbe7a9b87aed08bbd3";
...
const BASELINE_TESTS: &[SourceTestMapping] = &[
    ...
    SourceTestMapping {
        ...
        native: &["crates/workdeck-cli/src/install/download.rs#tests::downloads_use_packaged_names_and_cleanup_on_success_or_failure"],
    },
    SourceTestMapping {
        name: "installs beside the bundled skills so skill resolution still finds them",
        native: &["crates/workdeck-cli/tests/cli.rs#relocated_binary_resolves_packaged_skills_without_user_state"],
    },
    SourceTestMapping {
    ...
    SourceTestMapping {
        ...
        native: &["crates/workdeck-cli/src/install.rs#tests::platform_detection_matches_both_pinned_shell_oracles"],
    },
    SourceTestMapping {
        name: "refuses every visible and inactive-nvm competing install with exact remediation",
        native: &["crates/workdeck-cli/src/install.rs#tests::inactive_nvm_installations_are_discovered_before_mise_without_execution", "crates/workdeck-cli/src/install.rs#tests::install_conflict_gate_requires_explicit_force_without_mutating_candidates"],
    },
    SourceTestMapping {
        name: "names when the managed target shadows a competing install",
        native: &["crates/workdeck-cli/src/install.rs#tests::conflict_details_report_inferred_owner_and_all_path_order_states"],
    },
    SourceTestMapping {
        name: "uses canonical PATH identities for shadowing through a managed-directory alias",
        native: &["crates/workdeck-cli/src/install.rs#tests::manager_shaped_alias_is_preferred_without_changing_first_path_or_identity", "crates/workdeck-cli/src/install.rs#tests::identity_handles_missing_binary_and_path_order_without_writes"],
    },
    SourceTestMapping {
        name: "reports one conflict when PATH contains directory aliases to the same foreign install",
        native: &["crates/workdeck-cli/src/install.rs#tests::path_observation_deduplicates_identity_but_retains_aliases_and_order"],
    },
    SourceTestMapping {
        name: "uses a canonical manager path when an unrecognized alias is discovered first",
        native: &["crates/workdeck-cli/src/install.rs#tests::manager_shaped_alias_is_preferred_without_changing_first_path_or_identity"],
    },
    SourceTestMapping {
        name: "does not treat a PATH symlink to the managed binary as another install",
        native: &["crates/workdeck-cli/src/install.rs#tests::identity_handles_missing_binary_and_path_order_without_writes"],
    },
    SourceTestMapping {
        name: "allows the scripted force environment variable",
        native: &["crates/workdeck-cli/src/install.rs#tests::install_conflict_gate_requires_explicit_force_without_mutating_candidates"],
    },
    SourceTestMapping {
...
fn source_blob(repo: &Path, commit: &str, path: &str, bytes: usize, lines: usize, sha: &str) -> Result<String> {
    let source = crate::git_stdout_bytes(repo, ["show", &format!("{commit}:{path}")])?;
    ensure!(source.len() == bytes, "pinned {path} {commit} changed size: {} != {bytes}", source.len());
    ensure!(
        source.split(|byte| *byte == b'\n').count() == lines + 1,
        "pinned {path} {commit} changed line count"
    );
    ensure!(format!("{:x}", Sha256::digest(&source)) == sha, "pinned {path} {commit} changed SHA-256");
...
fn source_function_names(source: &str) -> Vec<String> {
    source
        ...
        .filter_map(|line| {
            ...
            (!name.is_empty() && name.chars().all(|char| char.is_ascii_alphanumeric() || char == '_'))
                .then(|| name.to_owned())
        })
        .collect()
}
...
fn verify_native_anchor(repo: &Path, item: &str, sources: &mut HashMap<String, syn::File>) -> Result<()> {
...
fn verify_oracle(repo: &Path) -> Result<()> {
    ...
    let array = value.as_array().context("install platform oracle is not an array")?;
    ensure!(array.len() == 20, "install platform oracle has {} cases, expected 20", array.len());
    ...
    for case in array {
        for field in ["pin", "os", "arch", "translated", "exit_code", "stdout", "stderr", "output"] {
            ensure!(case.get(field).is_some(), "install platform oracle case lacks {field}");
        }
    }
    Ok(())
}
...
fn verify_test_mapping(repo: &Path, source: &str, expected: &[SourceTestMapping]) -> Result<()> {
    ...
    ensure!(actual == expected_names, "pinned install test surface changed: {actual:?}");
    let mut unique = BTreeSet::new();
    ensure!(expected_names.iter().all(|name| unique.insert(*name)), "install test mapping repeats a source test");
    let mut rust_sources = HashMap::new();
    for mapping in expected {
        ensure!(!mapping.native.is_empty(), "install test {} has no native evidence", mapping.name);
...
pub(crate) fn verify(repo: &Path, baseline: &str) -> Result<()> {
    ensure!(baseline == BASELINE, "install script verifier received unexpected baseline {baseline}");
    let install = source_blob(repo, BASELINE, INSTALL_PATH, INSTALL_BASELINE_BYTES, INSTALL_BASELINE_LINES, INSTALL_BASELINE_SHA256)?;
    let stable_install = source_blob(repo, STABLE, INSTALL_PATH, INSTALL_STABLE_BYTES, INSTALL_STABLE_LINES, INSTALL_STABLE_SHA256)?;
    let tests = source_blob(repo, BASELINE, TEST_PATH, TEST_BASELINE_BYTES, TEST_BASELINE_LINES, TEST_BASELINE_SHA256)?;
    let stable_tests = source_blob(repo, STABLE, TEST_PATH, TEST_STABLE_BYTES, TEST_STABLE_LINES, TEST_STABLE_SHA256)?;

    let expected_baseline_functions = [
        "info", "warn", "fail", "usage", "detect_os", "detect_arch", "download", "fetch",
        "installed_version", "canonical_executable_path", "add_hunk_candidate", "preferred_manager_path",
        "shadowing_direction", "competing_install_channel", "competing_install_remediation",
        "check_competing_installs", "add_path_line", "squote", "first_existing", "main", "cleanup",
    ];
    let expected_stable_functions = [
        "info", "warn", "fail", "usage", "detect_os", "detect_arch", "download", "fetch",
        "installed_version", "add_path_line", "squote", "first_existing", "main", "cleanup",
    ];
    ensure!(source_function_names(&install) == expected_baseline_functions, "pinned installer baseline function surface changed");
    ensure!(source_function_names(&stable_install) == expected_stable_functions, "pinned installer stable function surface changed");
    for source in [&install, &stable_install] {
        for marker in INSTALL_MARKERS {
            ensure!(source.contains(marker), "pinned installer source is missing required marker {marker:?}");
        }
    }
    for marker in BASELINE_INSTALL_MARKERS {
        ensure!(install.contains(marker), "pinned installer baseline is missing required marker {marker:?}");
        ensure!(!stable_install.contains(marker), "pinned installer stable unexpectedly retains marker {marker:?}");
    }
    for source in [&tests, &stable_tests] {
        for marker in TEST_MARKERS {
            ensure!(source.contains(marker), "pinned installer test source is missing required marker {marker:?}");
        }
    }
    ...
    for marker in [
        ...
        ensure!(docs.contains(marker), "install script migration documentation is missing {marker:?}");
    }
    Ok(())
}
