...
This document defines the implementation and migration contract for Workdeck's reusable local session
...
approval UI, exactly-once execution, and a generic resource protocol without a second non-Workdeck
consumer. Workdeck retains review documents,
...
Publish one package, `@workdeck/session-broker`, with protocol/state, daemon/connection, managed-host,
...
`serveSessionBrokerDaemon`. Workdeck composes the package; the package never imports `src/*`.

Publication requires verified npm `@workdeck` scope ownership and trusted publishing. An approved
...
by Workdeck plus one clean external reference consumer. During `0.x`, public API breaks require a minor;
after `1.0.0`, a major.
...
Reverse-DNS form such as `dev.workdeck` is recommended. Every producer, caller, discovery record,
...
untrusted, size-bounded metadata conventions. They never confer authority. Workdeck's adapter continues
...
deployment policy, discovery design, and new security review. Workdeck may temporarily isolate its
unsupported `HUNK_MCP_UNSAFE_ALLOW_REMOTE` escape hatch.

Workdeck browser review keeps independent per-session/per-review capabilities and same-origin/no-CORS
...
this changes Workdeck's fixed default port, Phase 4 must stop for explicit approval; denial requires a
...
Project-boundary/VCS discovery remains app-owned; the SDK knows no `.git`, `.workdeck`, provider, or
extension semantics. Selector metadata remains untrusted, bounded, and non-authoritative.

## Workdeck composition and migration

Workdeck owns its session schemas/projections/errors; review documents/publication generations,
...
discovery; and executable launch/upgrade copy. Generic state removes Workdeck-shaped projections; Workdeck
builds them from read-only generic entries/events, and the package never imports Workdeck types.

| Existing contract                                    | Migration requirement                                                                                                                                                 |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `HUNK_MCP_HOST`, `HUNK_MCP_PORT`, `HUNK_MCP_DISABLE` | Preserve in Workdeck adapter for at least one documented minor window.                                                                                                    |
| `HUNK_MCP_UNSAFE_ALLOW_REMOTE`                       | Temporary unsupported Workdeck-only escape hatch; never generic.                                                                                                          |
| runtime directory `workdeck-mcp`                         | Preserve or dual-read so old/new binaries cannot race separate namespaces.                                                                                            |
...
| incompatible daemon                                  | Never signal from unverifiable PID. Interactive Workdeck waits and retries until the incumbent exits while idle; forced replacement requires an authenticated generation. |
...
An interactive Workdeck window keeps reviewing locally while an incompatible or pre-authentication
...
Workdeck's fixed-endpoint Phase-1 credential store uses a home-local `.workdeck` parent when
`XDG_RUNTIME_DIR` is unavailable, rather than a predictable name in a shared temporary directory.
It inherits the current user's ACL when it creates the `workdeck-mcp/security-v1` directory
...
Before publishing even `initializing`, a Workdeck candidate binds and retains the legacy guard endpoint;
...
- Workdeck env/path/guard/selectors/routes, old-client refusal, and mixed-version fail-closed flows remain
  tested.
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | Fix connection ownership, signed producer/caller authentication, fixed Workdeck `appId`, broker revision `1`, current Workdeck app revision as a singleton, empty generic features, authorization, validation, and bounds without an unauthenticated transition. |
| 2     | Achieve Node/Bun frame, size, shutdown, and machine-sleep parity.                                                                                                                                                                                        |
| 3     | Generalize identity/ranges/features/descriptors/registration/selectors while preserving Workdeck's developer-tools plugin; prove Phase 1 is its one-element wire case.                                                                                       |
...
| 7     | Adopt in Workdeck, remove obsolete paths, preserve semantics, run end-to-end flows, reconcile historical TODOs, and complete independent review.                                                                                                             |
