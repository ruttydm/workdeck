...
Defines the target import boundaries between Workdeck's top-level source trees and records what the
...
- `src/workdeck-review` imports `src/session/agent`: the skill document is generated from the agent
  surface by design.
...
(`commandCatalog`), the invocation errors (`errors` — "a failure Workdeck raises because of how it
...
teardown (`shutdown`), `.workdeck`/VCS project-root discovery (`projectRoot`), the atomically
...
per-channel release lookup (`latestRelease`), and the `workdeck update` execution (`selfUpdate`).
...
- **`src/core/cli.ts` → `src/app/cli.ts`.** CLI parsing that registers every tier's command
  surface (including `workdeck session *` from `session/agent/surface.ts`) is composition, not
