...
# Workdeck extensions: an exploration

What a JavaScript/TypeScript extension system for Workdeck could look like, modeled
on the extension system of [pi](https://pi.dev) (`@earendil-works/pi`), grounded
in Workdeck's actual architecture. This is a design exploration, not a spec.
...
- **Packages bundle extensions + skills + prompts + themes** and are installed
  with `pi install`. Notably, Workdeck already participates in this ecosystem from
  the other side: our `package.json` declares `"pi": { "skills": ["./skills"] }`
  so pi users get the `workdeck-review` skill automatically.

## 2. Why this fits Workdeck

Workdeck's positioning — "modern desktop diff tool in a terminal", review-first,
...
The same force works for us twice over: agents can write Workdeck extensions, and
Workdeck is _used to review agent changesets_, so extensions that connect review to
agent workflows (post a comment back to the agent session, re-run the agent on
a rejected workdeck) are the most natural third-party contributions imaginable.

## 3. Example extensions (use-case catalog)

Roughly ordered from "certain someone builds this in week one" to speculative:

- **Open-in-X**: a key that opens the selected workdeck in `$EDITOR`, VS Code,
  IntelliJ, or a browser at the forge URL. (Core has one editor action today;
  everyone's variant differs.)
- **Copy-as**: copy the selected workdeck as a fenced patch, a GitHub-suggestion
...
- **Forge review bridge**: publish accumulated review notes as a GitHub /
  GitLab / Gerrit review from inside Workdeck; show existing PR review threads as
  notes beside the hunks they target.
- **Send-to-agent**: a shortcut that packages the selected workdeck plus your draft
  note and sends it to a running pi / Claude Code session ("fix this"), using
  the daemon the same way `workdeck session comment` does — in reverse.
...
- **Explain-this-workdeck**: call the user's own model API key to annotate a workdeck
...
## 4. What Workdeck already has (integration-surface audit)
...
- **`MenuId` is a closed union**, though menu _entries_ are already data-shaped
  ...
  actions exist.
- **No dynamic user-code loading anywhere**, and Workdeck ships as a compiled Bun
...
- Discovery: `~/.config/workdeck/extensions/*.ts` and `*/index.ts` (global, follows
  our existing XDG path logic in `src/core/run/paths.ts`), `.workdeck/extensions/`
...
4. **Phase 3 — protocol surfaces.** Session-command registry (after collapsing
   ...
   need a capability story in the daemon handshake so `workdeck session <ext-cmd>`
   fails cleanly against a session that lacks the extension.
5. **Phase 4 — distribution.** `workdeck install npm:...` package support,
   ...
   is a `workdeck-extension-authoring` skill — same trick we already use for
   `workdeck-review`).

## 7. Open questions

- **Compiled-binary loading.** The installed `workdeck` is a `bun build --compile`
...
- **Security posture.** Extensions run with full user permissions (same as pi;
  ...
  that repo's code without an explicit prompt. This matters _more_ for Workdeck
...
- **API stability.** pi treats extension API churn as acceptable pre-1.0. We
  should version the API (`workdeck.apiVersion`) from day one, and keep the
...
The pi model transplants well. Workdeck's architecture is closer to extension-ready
