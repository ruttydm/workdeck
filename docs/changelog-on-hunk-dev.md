...
# Release notes on workdeck.dev

`CHANGELOG.md` is the source of truth for what shipped. `scripts/generate-changelog.ts` projects it
into the pages published at `workdeck.dev/changelog`, the same way `scripts/generate-docs.ts` projects
...
Workdeck publishes 40-plus releases at roughly two a week, and before this that content existed only in
`CHANGELOG.md` and GitHub Releases — about a quarter as much prose as the whole docs site, none of it
on workdeck.dev. Three things follow from hosting it here:
...
- **Answer engines can finally answer version questions.** The site already ships `llms.txt`,
  ...
  excluded from `llms-small.txt`, which stays scoped to learning Workdeck.
- **Release videos get a permanent home.** `skills/workdeck-launch-video` renders the real TUI for every
...
Step 4 of `skills/workdeck-release/SKILL.md` regenerates after every tag, including backports. Every tag
...
- **The in-app update notice.** `src/core/process/updateNotice.ts` tells users a new version exists without
  linking what changed. Appending `workdeck.dev/changelog/<minor>` is the highest-intent entry point
