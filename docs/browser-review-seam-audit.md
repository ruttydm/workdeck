> Native migration note: this page preserves the pinned upstream document's complete headings, examples, decisions, and limitations under Workdeck naming. It is documentation, not an executable source mirror. Runtime ownership is Rust/Ratatui; use the canonical native crates, tests, and companion guides linked below for current behavior. Historical upstream tool names remain only where the document records provenance or a limitation.
...
- **A1. Collapsed-gap line ranges — 3 implementations, terminal off by one.** Terminal
  ...
  objects. For a pure-insertion workdeck the terminal's leading range is off by one against core's,
  ...
  `pierre.ts` copies deleted; fixtures `pure-insertion-workdeck` and `pure-deletion-workdeck` in
  ...
  anchor side has zero rows, so `pure-deletion-workdeck` pins lines 1–5 instead of preserving the
...
- **A2. Trailing-context existence — 3 formulations.** `pierre.ts` `trailingCollapsedLines`,
  ...
  in that function: a last workdeck with a zero-count side leaves the two line-array tails one apart,
...
- **A3. Per-side workdeck range — 4 implementations, one wrong.** `core/liveComments.ts`
...
- **A6. Workdeck content-index rebasing — 2 copies, opposite `isPartial` conclusions.** Web
  `pierreDocument.ts` `isolatePierreHunk` vs terminal `sourceBackedHighlight.ts` (~:108-199).
  Fix: one `rebaseReviewHunk(workdeck, origins)` in core.
  ...
  validate without re-walking; the browser's isolate-one-workdeck use lands on it in Phase 5.
- **A7. File split/unified line totals — web guesses.** `pierreDocument.ts` reconstructs
  ...
  workdeck spans. Fix: carry both on `ReviewFileV1`.
...
- **A10. Default line target for a workdeck note — 2 divergent rules.** Terminal
  `core/liveComments.ts` `firstCommentTargetForHunk` (first added line, else first deleted);
  web `App.tsx` (workdeck's first line — usually context). Same action anchors to different lines
  per client. Fix: `reviewDefaultHunkLineTarget(workdeck)` in core, used by both (and by reveal).
  ...
  pins the target, `pure-deletion-workdeck` and `workdeck-with-leading-context` adversarially.
...
duplication); workdeck header text (browser delegates to Pierre separators); platform hashing.

## B. Navigation and selection

- **B1. Workdeck navigation across the stream — 3 implementations.** Terminal
  ...
  (`scope: workdeck|file|annotated-workdeck|annotated-file`, delta, wrap policy) planned over shared
  ...
  multi-step carry rule it lacked applies there too. Fixture `annotated-workdeck-multi-step-carry`
...
- **B2. Wrap vs clamp policy split.** `moveToFile` clamps; `moveToAnnotatedFile` wraps
  ...
  workdeck/file/annotated-workdeck clamp, annotated-file wraps — and `findNextAnnotatedFile` is deleted.
  The asymmetry is today's terminal behavior kept deliberately, including two quirks now stated
  rather than implied: a clamping workdeck move at an edge re-selects and re-reveals the same workdeck
...
- **B3. File-jump semantics — hard-coded identically in both clients.** "Workdeck 0 + file-top
  ...
  _Repaid (Phase 1 PR 3, core and terminal sites)_: `selection/select-file` owns "first workdeck"
  (`REVIEW_FILE_JUMP_HUNK_INDEX`) with `REVIEW_FILE_JUMP_REVEAL` as its default reveal, and the
  forward-cross-file alignment rule moved into the workdeck-move planner, where the crossing is
  known. The terminal's `selectFile` lowers to the intent and no longer takes a workdeck index.
...
- **B6. Reveal-target derivation — web re-derives, wrongly.** Web `App.tsx` recomputes the
  workdeck target line (`newRange ? "new" : "old"`), duplicating core `canonicalLineForHunk` which
  ...
  workdeck's position is its first row while a note about the whole workdeck hangs from its first change.
...
- **B7. "Jump to note" target — terminal geometry decides, web ignores.** The
  ...
  draft in the selected workdeck first, else the note anchored earliest in it, arrival order breaking
...
- **B8. Notes-by-workdeck grouping — web re-filters by range containment.** `ReviewStream.tsx`
  ...
  a note under _every_ workdeck it overlaps, so converting it changes what a reviewer sees and
...
- **B11. Viewport-driven selection — opposite policies.** Terminal publishes
  nearest-workdeck-to-center into shared state; web keeps IntersectionObserver results local. With
  ...
  viewport-centered workdeck selection and its line-cursor anchoring both publish through it, and the
...
- **C5. Reconnect/backoff — 4 schedulers, 1 verbatim duplicate.** `apiClient.ts` (exp/4 s),
  ...
  `createReconnectScheduler` in `@workdeck/session-broker-core`; EventSource's built-in reconnect
...
- **D4. Canonical-file ↔ manifest consistency — 3 checks, 3 field lists.** Producer
  ...
  canonical workdeck _content_ to manifest hunks. Fix: one order-independent
  ...
  key order never participates, and workdeck content is checked alongside the geometry derived
  from it. The manifest gained the content that requires (patch text, workdeck blocks, source
...
- **F2. Semantic command effects are closures instead of intent dispatches.** The ~15
  review-semantic commands (workdeck/file/annotated navigation, start note, toggle gap, toggle
  ...
  through the existing apply-action path, and the agent runtime's `workdeck session` surface
  ...
  an optional client-measured line, falling back to the shared whole-workdeck default;
...
- **G1. View defaults and option classification.** Terminal view options resolve through the
  layered defaults chain (built-ins → user config → repo `.workdeck/config.toml` → command
...
- **G3. Semantic addressing / permalinks.** The prototype's URL fragment carries only the
  capability token — there is no grammar for addressing a file/workdeck/line/note. Three consumers
  will need one: browser deep links and back/forward history, a terminal "copy link" command,
  and agent surfaces already addressing targets by file/workdeck. One serialize/parse address
...
- **G4. User-facing error catalog.** The repo already solves this once for agents:
  ...
  keeps its own wording — its codes are `workdeck session` CLI failures rather than these, and
