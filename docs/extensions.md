> Native migration note: this page preserves the pinned extension guide's complete contract, examples, and caveats under Workdeck naming. It is documentation, not an executable source mirror. Current runtime ownership is the Rust/Ratatui extension API and host; consult [the native API reference](/docs/extend/extension-api/), [extension architecture](extension-architecture.md), and [bundled extension guides](/docs/extend/extensions/) for the authoritative implementation. Historical tool names remain only when recording provenance or a limitation.

# Writing Workdeck extensions

A Workdeck extension entry is one TypeScript (or JavaScript) file that
default-exports a function. Workdeck imports it at startup and hands it an API
object. An entry may stand alone or be declared by a folder's optional
`package.json` manifest; no build step is required.

```ts
// ~/.config/workdeck/extensions/hello.ts
import type { HunkExtensionAPI } from "hunkdiff/extension";

export default function (workdeck: HunkExtensionAPI) {
  workdeck.on("startup", (_event, ctx) => {
...
> changes will be called out in release notes, and `workdeck.apiVersion` identifies
> the surface an extension was written against.

Writing one with a coding agent? `workdeck skill path workdeck-extensions` prints a
bundled skill that maps the touchpoints below for agents, the way
`workdeck skill path` does for reviewing.

## Where Workdeck looks for extensions
...
| 3     | `~/.config/workdeck/extensions/`                         | runs immediately      |
| 4     | `.workdeck/extensions/` in the repo under review         | **prompts for trust** |
| 4     | `[extensions] paths` in the repo `.workdeck/config.toml` | **prompts for trust** |
...
everywhere). The manifest field is `workdeck`:

```text
~/.config/workdeck/extensions/my-ext/
  package.json          # {"workdeck": {"extensions": ["./src/index.ts"]}}
...
The `workdeck` field may also state the minimum extension API version the folder
needs:
...
```json
{
  ...
  "workdeck": { "extensions": ["./src/index.ts"], "apiVersion": 3 }
}
```

A Workdeck whose extension API is older than `apiVersion` refuses the folder with a
...
as `workdeck.apiVersion`). The standard `name`, `version`, and `description` fields
...
`workdeck`, `git`, `jj`, and `sl` are reserved for what Workdeck ships. An extension
...
[source order](#where-workdeck-looks-for-extensions) loads and the other is skipped the
...
publish to. `workdeck extension install` clones one into a managed directory
(`~/.config/workdeck/extensions/installed/<repo-name>/`), verifies it actually
...
workdeck extension install https://github.com/acme/workdeck-word-diff.git
workdeck extension install ~/dev/workdeck-word-diff         # a local checkout, for testing
```
...
`workdeck extension list` shows every managed install with its version, commit, and
source. `workdeck extension update [name]` re-clones one install (or all of them)
from its recorded source — an install pinned with `@ref` stays at that ref
until you re-install with a different one. `workdeck extension remove <name>`
deletes the install and its record. Managed installs never collide with
extensions you copied into `~/.config/workdeck/extensions/` by hand, and the
...
```text
workdeck-word-diff/
  package.json          # name, version, description, workdeck field
  index.ts              # or entries declared in "workdeck": {"extensions": [...]}
  README.md
```
...
1. Give `package.json` a real `name`, `version`, and `description`, declare
   entries under the `workdeck` field, and state `"workdeck": {"apiVersion": N}` if you
   rely on recent API surface (see [the manifest](#where-workdeck-looks-for-extensions)).
...
3. Tag releases (`v1.2.0`) so users can pin with `@v1.2.0` instead of tracking
   your default branch.
4. Push the repository to any git host and add the **`workdeck-extension`** GitHub
   topic so people can find it: every public repository with that topic shows
   up at <https://github.com/topics/workdeck-extension>. Opening a pull request
   against `website/src/data/extensions.ts` also lists it on
   <https://workdeck.dev/extensions>.

Before publishing, exercise the exact layout users will install:
`workdeck extension install /path/to/your/checkout` installs from a local
repository, and `workdeck diff --extension /path/to/your/checkout` loads it for one
...
So repo-local sources are gated. The first time Workdeck finds extensions in a
repository's `.workdeck/extensions` (or repo-config `paths`), it skips them and asks:
...
- **Never** records a denial so Workdeck stops offering.

Decisions are stored per repository root in `~/.config/workdeck/state.json`. The
...
factory is running; Workdeck seals the object afterwards so a deferred callback
...
The API generation this Workdeck speaks (currently `15`). Branch on it if you want
one file to support several Workdeck versions. Version 15 adds `{ side, line }` to
...
### `workdeck.registerCliCommand(command, handler)`
...
Bare `workdeck --help` remains static and does not load extensions. The extension
owns `workdeck <name> --help` and receives `--help` unchanged.

`summary` and `usage` are what Workdeck shows when a top-level token reaches
...
```text
workdeck: Unknown command: nosuchthing

Extension commands available here:
workdeck cli-tools <status|review> [args...] — Demonstrate extension-provided CLI workflows
workdeck gh <number|owner/repo#number|pull-request-url> [--repo <owner/repo>] — Review a GitHub pull request
```
...
### `workdeck.configureSession(options)`
...
```ts
workdeck.configureSession({ viewPreferences: "transient" });
```
...
### `workdeck.registerFileLanguage(matcher, language)`
...
```ts
workdeck.registerFileLanguage(".zig", "zig");
workdeck.registerFileLanguage({ kind: "extension", value: "bzl" }, "python");
workdeck.registerFileLanguage({ kind: "filename", value: "BUILD" }, "python");
workdeck.registerFileLanguage(
  { kind: "glob", value: "generated/**/*.proto", target: "path" },
  "protobuf",
);
workdeck.registerFileLanguage({ kind: "glob", value: "*.component", target: "basename" }, "typescript");
```
...
the basename or the review path exactly as Workdeck decoded it. `/` is the review-path separator;
...
Workdeck's reserved `.mts` and `.cts` mappings run first and cannot be overridden. Otherwise, exact
...
### `workdeck.registerVcsAdapter(adapter)`

Contribute an additional VCS backend. This is the same call Workdeck's own bundled
Git, Jujutsu, and Sapling backends make.

```ts
workdeck.registerVcsAdapter({
  id: "hg",
  name: "Mercurial",
...
reports as unknown and Workdeck synthesizes the added-file diffs for you, skipping
...
What detection never overrides is an explicit choice: a `vcs = "<id>"` in Workdeck
...
provider Workdeck has never seen because `.workdeck` itself establishes the project root;
...
candidates, Workdeck extends the provisional registry instead of executing its
already loaded factories again. If repo config changes an existing extension's
factory config, Workdeck sends that provisional instance `shutdown` before rebuilding it.
...
- `watchPlan(input, ctx)` — the filesystem targets that cover that state, so
  Workdeck reacts to events instead of polling on a timer.
...
`readFileSource` and Workdeck will expand context past the workdeck, highlight against
the real file, and word-diff accurately.

```ts
async load(input, ctx) {
  // Pin the revisions while the operation loads, then close over them: by the
  // time Workdeck asks for a file, nothing can have moved underneath it.
...
resource limit; Workdeck shows expansion as unavailable without treating the result
as an extension failure. Workdeck calls the reader
...
opaque `sourceCacheKey` so Workdeck can reuse its highlighted output. An equal per-file
...
stable identity and Workdeck will invalidate conservatively. Leaving
`readFileSource` off is fine: Workdeck falls back to the content the patch itself carries,
...
the order they should appear. Each entry is one of two kinds, and Workdeck builds
...
VCS produces better text for a file than Workdeck reading the working copy would —
...
A **skipped** entry is a file Workdeck should list but not render. Reviewing a
...
VCS calls unknown and Workdeck synthesizes the added-file diffs from the working
...
Workdeck reads move classes back out of the patch itself, so emit ANSI-colored diff
...
Throw a `HunkExtensionUserError` when the problem is how Workdeck was invoked rather
than a bug — no repository here, an unresolvable revision, a missing binary.
Workdeck prints the message without a stack trace and lists the suggestions beneath
it. Anything else is reported as an unexpected error.

```ts
import { HunkExtensionUserError } from "hunkdiff/extension";

throw new HunkExtensionUserError("`workdeck stash show` is not supported by Mercurial.", {
  suggestions: ["Use `workdeck show <rev>` to review a commit instead."],
});
```

Workdeck detects this structurally — an object whose `name` is
...
rather not hard-code the string. Workdeck's own bundled Git, Jujutsu, and Sapling
backends raise their failures exactly this way.

### `workdeck.registerPane(pane)`
...
```tsx
// ~/.config/workdeck/extensions/flat-pane.tsx
...
export default function (workdeck: HunkExtensionAPI) {
  workdeck.registerPane({
    id: "flat",
    title: "Flat files",
    placement: "right",
    component: FlatPane,
  });
  workdeck.registerCommand({ id: "toggle-flat", title: "Toggle flat pane", key: "ctrl+f" }, (ctx) => {
...
must be greater than `0` and at most `1`; Workdeck rounds that fraction of the full
...
extensions that use `fraction` should declare `"workdeck": { "apiVersion": 12 }` in
their manifest.

Use `defaultOpen` to open a pane initially, `replaces: "workdeck:files"` to replace
...
`"<extensionId>:<paneId>"` key, and Workdeck follows those replacement chains.

`workdeck:files` is a named role, not a left-edge location. The
`workdeck.view.toggleFilesPane` command (`s` by default) and **View → Files pane**
...
`workdeck.view.toggleFilesPane` apply to the resolved slot in the usual way. The
former `workdeck.view.toggleSidebar` id remains a compatibility alias.
...
(context rows use Workdeck's canonical new-side). The installable
[Workdeck Lens](https://github.com/modem-dev/workdeck-lens) extension uses the painter; a
blame or diagnostic pane can use the address without waiting for a keypress.
Install the lens with `workdeck extension install modem-dev/workdeck-lens`.

Import `react` normally — Workdeck serves its own React instance to extension files
...
| `selectedHunkIndex` | the selected workdeck within that file, or `null`                                                                                                                             |
...
`ExtensionPane*`, `ctx.panes`, and `replaces: "workdeck:files"` in new code.
...
The three workdeck surfaces line up by design: each file's `hunks` lists public
...
`actions.selectHunk(fileId, hunkIndex)` accepts it. That is everything a workdeck
checklist, a per-workdeck progress view, or an agent-annotation navigator needs —
match an annotation's `oldRange`/`newRange` against the summaries' spans to
find its workdeck — without touching the opaque `metadata`.
...
export function handlePaneKey(props: ExtensionPaneProps, key: ExtensionKeyEvent) {
  ...
  if (nextFile && props.keybindings.matches(key, "workdeck.review.nextFile")) {
...
Workdeck commands and extension commands under their documented ids, and its key
...
Workdeck owns pane geometry, dividers, and responsive omission. Render failures are
contained to that pane; a failed `workdeck:files` replacement restores file
navigation.

Props carry the pane's exact `width` and `height`. Use a `<scrollbox>` ref for
scroll position and selection following; Workdeck serves the matching
...
function HunkList({
  ...
  return (
    <scrollbox ref={scrollRef} width="100%" height="100%" scrollY={true} focused={false}>
      {files.flatMap((file) =>
        (file.hunks ?? []).map((workdeck) => {
          const selected = file.id === selectedFileId && workdeck.index === selectedHunkIndex;
          return (
            <box
              key={`${file.id}:${workdeck.index}`}
              id={`row-${file.id}-${workdeck.index}`}
              style={{ width: "100%", height: 1 }}
              onMouseUp={() => actions.selectHunk(file.id, workdeck.index)}
            >
              <text
                content={` ${file.path}  ${workdeck.header}`}
...
whatever version Workdeck pins — a wider surface than `hunkdiff/extension` itself.
The built-in files pane uses the same calls, so changes that break this contract
break Workdeck first. Keep scroll handling small and behind your own helpers.
...
export default function (workdeck: HunkExtensionAPI) {
  workdeck.on("file_viewed", ({ file }) => markViewed(file.path));
  workdeck.registerPane({ id: "progress", component: ViewedCount });
}
```
...
### `workdeck.registerFileView(view)` (experimental)

A file view is an alternate **host-rendered** presentation of one file in the
same top-to-bottom review stream. It is not a whole-file React component: Workdeck
owns row measurement, scrolling/windowing, workdeck navigation, and fallback to
...
Rows may bind themselves to exact old/new source ranges so Workdeck can insert its
...
or loaded by default; copy the folder into `~/.config/workdeck/extensions/`, install
...
export default function (workdeck: HunkExtensionAPI) {
  workdeck.registerFileView({
    ...
    async layout(input) {
      ...
      return {
        rows,
        hunkRows: (input.file.hunks ?? []).map((workdeck) => ({
          startRow: Math.max(0, (workdeck.newRange?.[0] ?? 1) - 1),
          endRow: Math.min(rows.length - 1, (workdeck.newRange?.[1] ?? 1) - 1),
        })),
      };
    },
  });
...
complete old/new workdeck ranges remain available through `input.file.hunks`.
`readDocument("old" | "new")` is lazy and cached by Workdeck; it resolves exact
...
terminal attributes (`bold`, `italic`, `underline`, `strikethrough`). Workdeck
...
parsed workdeck needs one in-bounds, inclusive `hunkRows` entry at the same array
position as `input.file.hunks`.

A row's optional `sourceRanges` contains inclusive, one-based exact-source
bindings such as `{ side: "new", range: [12, 18] }`. Workdeck reads only the bound
...
presentation/workdeck target. Inline notes anchor by their existing preferred-side start
line and are inserted before the bound row. Placement is **all-or-raw** per
file: if any visible note is range-less or unbound, Workdeck temporarily renders
...
warning per concrete extension registration and fall back to raw diff. Rapid width changes are coalesced, and Workdeck never paints
...
propagation, while wheel, drag, and unhandled input remain host-owned. Workdeck
...
Workdeck treats `layout` as a pure derivation of `(file, width)` and reuses a
  ...
  workdeck.registerFileView({
    ...
    layout: ({ file }) => ({
      rows: (file.hunks ?? []).map((entry) => ({
        id: `workdeck:${entry.index}`,
        spans: [{ text: expanded ? `Workdeck ${entry.index + 1} · ${entry.header}` : entry.header }],
      })),
      hunkRows: (file.hunks ?? []).map((entry) => ({ startRow: entry.index, endRow: entry.index })),
    }),
  });

  workdeck.registerCommand(
...
workdeck.registerCommand({ id: "fold", title: "Fold this file", key: "f10" }, (ctx) => {
...
Add a `mode` when a file view needs keyboard input. Workdeck sends it keys after
...
workdeck.registerCommand({ id: "outline-keys", title: "Outline keys", key: "f9" }, (ctx) => {
...
- `"pass"` continues through any active session keyboard mode, then Workdeck's
...
never reaches `onKey`. Workdeck also exits when the selected file, active presentation,
...
### `workdeck.registerLineHighlighter(highlighter)`

A line highlighter marks **character ranges inside Workdeck's own diff rendering**
...
export default function (workdeck: HunkExtensionAPI) {
  workdeck.registerLineHighlighter({
    id: "todos",
    highlight({ file }) {
...
Workdeck's row model. A context line may be addressed through either side's line
...
invisible on an added line's green. Workdeck resolves each tinted tone against the
...
still paints, chosen against the surface Workdeck assumes rather than the one behind
...
the complete set of marks for one file — or `null` for none. Workdeck calls it per
...
```ts
workdeck.registerCommand({ id: "next", title: "Next match", key: "f9" }, (ctx) => {
...
workdeck.registerKeyboardMode({
  ...
  onKey: (key, ctx) => {
    if (key.sequence === "g") {
      if (pending === "g") {
        pending = "";
        ctx.commands.execute("workdeck.review.jumpToTop");
      } else {
        pending = "g";
      }
      return "handled";
    ...
    ctx.commands.execute("workdeck.review.stepDown");
    return "handled";
  },
});

workdeck.registerCommand({ id: "vim", title: "Toggle Vim navigation", key: "ctrl+v" }, (ctx) => {
...
- `"handled"` consumes the key.
- `"pass"` continues through ordinary Workdeck commands and focused scrolling.
...
Register a named command, optionally bound to a key. Commands share Workdeck's
...
export default function (workdeck: HunkExtensionAPI) {
  workdeck.registerCommand({ id: "hello", title: "Say hello", key: "ctrl+g" }, (ctx) => {
...
```ts
workdeck.registerCommand({ id: "hello", title: "Say hello", key: ["ctrl+g", "f9"] }, (ctx) => {
  ctx.notify("hello from a command");
});
```
...
is named `"<extensionId>.<commandId>"`, while Workdeck's own are `"workdeck.app.quit"`
...
- `ctx.panes.open(paneId)` / `close(paneId)` / `toggle(paneId)` — a bare id
  ...
  `"workdeck:files"` for the literal built-in pane. These controls address
  registrations directly; they do not resolve a replacement slot. To toggle
  whichever pane currently owns the files role,
  call `ctx.commands.execute("workdeck.view.toggleFilesPane")`. Opening a left/right
...
workdeck.registerCommand(
  ...
  (ctx) => {
    ...
    ctx.notify(hunkIndex === null ? file.path : `${file.path} — workdeck ${hunkIndex + 1}`);
  },
);
```
...
It belongs to this file and workdeck, uses Workdeck's canonical new-side address for a
...
`ctx.commands` invokes Workdeck's documented semantic commands through the exact same live command
table used by the keyboard, menus, and help:

```ts
workdeck.registerCommand({ id: "skip-three", title: "Skip three hunks", key: "ctrl+j" }, (ctx) => {
  if (ctx.commands.isEnabled("workdeck.review.nextHunk")) {
    ctx.commands.execute("workdeck.review.nextHunk", { count: 3 });
  }
});
```

Only explicitly public built-in `workdeck.*` commands can be invoked. Unknown, disabled,
...
`workdeck.review.alignCurrentLineTop`, `workdeck.review.alignCurrentLineCenter`, and
`workdeck.review.alignCurrentLineBottom` commands.

`count` defaults to `1` and must be a positive safe integer no greater than `10,000`. Relative
line, viewport, horizontal, file, workdeck, and annotated navigation applies the count atomically in
...
`{ fileId }`-scoped. See
[`workdeck.registerLineHighlighter`](#hunkregisterlinehighlighterhighlighter).
...
export default function (workdeck: HunkExtensionAPI) {
  workdeck.registerCommand({ id: "publish", title: "Publish review" }, async (ctx) => {
...
the selection, and a workdeck index is clamped into the file's real range.
...
sits on. A workdeck hundreds of lines tall has one anchor, so `selectHunk` can leave
the line you meant pages below the viewport; `revealLine` scrolls to the line
itself, lands it a little below the viewport top like every other Workdeck reveal,
...
(`view.cursor_line = "off"`) — the jump lands on the workdeck containing the line
instead. Only a line no workdeck of the file covers is refused, with a warning
naming your extension, and so are a side outside `"old"`/`"new"` and a line
number that is not a positive whole number.

```ts
workdeck.registerCommand({ id: "first-todo", title: "Jump to the first TODO" }, async (ctx) => {
...
which workdeck of the selected file to jump to, then navigating there:

```ts
workdeck.registerCommand({ id: "pick-workdeck", title: "Pick a workdeck", key: "ctrl+k" }, async (ctx) => {
  const file = ctx.selection.file;
  ...
  const labels = hunks.map((workdeck) => workdeck.header || `workdeck ${workdeck.index + 1}`);
  const picked = await ctx.dialogs.select({ title: "Which workdeck?", options: labels });
...
Workdeck draws the dialog, not you: your text fills the title, body, and choices,
...
itself as Workdeck asking. Workdeck's own bundled extensions omit that redundant marker.
...
```ts
workdeck.registerCommand({ id: "shout-headings", title: "Shout headings", key: "f7" }, async (ctx) => {
  const file = ctx.selection.file;
...
`workdeck show HEAD` is the file at that commit, not the working-tree file. It
returns `null` when the file or side is absent, no source is available, reading
fails, or the document exceeds Workdeck's size limit. Reads never prompt. An invalid
side rejects the promise.

Writes require all of the following:

- an unstaged working-tree review (`workdeck diff` with no revision range)
...
### `workdeck.transformChangeset(fn)`
...
```ts
workdeck.transformChangeset((changeset) => ({
  ...changeset,
...
read-only views Workdeck hands outward (event payloads, pane props, a command's
...
| `hunk_viewed`          | `{ file, hunkIndex }`   | when selection settles on a different workdeck                |
...
`hunk_viewed` is the workdeck-grain sibling of `file_viewed`. It fires when the
settled `(file, workdeck)` pair changes — including `[`/`]` inside one file — and
does not fire for current-line movement within a workdeck. `file_viewed` still fires
only when the selected file object changes, so a soft reload can report a fresh
file without counting as a new workdeck read.
...
`note_created` and `note_edited` cover notes authored in Workdeck's own UI, in this
...
`shutdown` handlers get a short window (250ms) to finish before Workdeck replaces
...
`workdeck.events` is a small bus shared by every loaded extension. Use it to
...
export default function (workdeck: HunkExtensionAPI) {
  workdeck.events.on<{ fileCount: number }>("summary:ready", (payload, ctx) => {
    if (payload.fileCount > 100) ctx.panes.open("summary");
  });

  workdeck.on("changeset_loaded", ({ changeset }, ctx) => {
    workdeck.events.emit("summary:ready", { fileCount: changeset.files.length });
...
Your extension's own `[extension.<id>]` config table, as a plain object. Workdeck
...
> workflow, and Workdeck shows a startup notice listing the extension ids a repo
> configures — but it means `workdeck.config` must never be trusted for
...
```ts
const patterns = (workdeck.config.patterns as string[] | undefined) ?? ["*.lock"];
```
...
- [Workdeck Lens](https://github.com/modem-dev/workdeck-lens) for opaque selected-row
  paint (`workdeck extension install modem-dev/workdeck-lens`).
...
```ts
// ~/.config/workdeck/extensions/collapse-generated.ts
import type { HunkExtensionAPI } from "hunkdiff/extension";
...
export default function (workdeck: HunkExtensionAPI) {
  const patterns = (workdeck.config.patterns as string[] | undefined) ?? [
    "*.lock",
    "*-lock.json",
    "dist/*",
  ];

  workdeck.transformChangeset((changeset, ctx) => {
...
```bash
workdeck diff --extension ./collapse-generated.ts
```

## CLI flags and config reference

```bash
workdeck diff --extension ./path/to/entry.ts   # load one entry file for a review (repeatable)
workdeck diff --extension ./my-ext             # a folder extension: loads ./my-ext/index.ts
workdeck --extension ./my-ext cli-tools status # load then run its top-level CLI command
workdeck --no-extensions cli-tools status      # no discovery or import; command is unavailable
workdeck diff --no-extensions                  # disable user extensions for this review
```
...
paths = ["~/dev/workdeck-ext/index.ts"] # extra entry files or directories

[extension.my-extension]            # opaque payload handed to that extension
some_key = "some value"
```

`[extensions] enabled` layers like every other option: a repo `.workdeck/config.toml`
overrides your user config. `--no-extensions` is a hard off switch that no config
layer can re-enable. Both govern **user** extensions only — Workdeck's bundled
Git, Jujutsu, and Sapling backends load either way. `[extensions] paths` from a repo
config is trust-gated the same way `.workdeck/extensions` is, because it is
