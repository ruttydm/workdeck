...
export const DAEMON_BUILD_MISMATCH_PREFIX = "The Hunk session daemon is from";

/** Describe one build for a message, disambiguating equal package versions by revision. */
function describeMismatchBuild(build: DaemonBuildMismatchBuild, other: DaemonBuildMismatchBuild) {
...
export function daemonBuildMismatchMessage(details: DaemonBuildMismatchDetails) {
  if (!details.daemon) {
    const launch = details.launch
      ? ` (pid ${details.launch.pid}, started ${details.launch.launchedAt} from ${details.launch.command})`
      : "";
    return `${DAEMON_BUILD_MISMATCH_PREFIX} an older build that predates \`hunk daemon status\` and refuses this CLI${launch}.`;
  }
  const relation = details.recommendedAction === "restart-daemon" ? "an older" : "a newer";
  return (
    `${DAEMON_BUILD_MISMATCH_PREFIX} ${relation} build (daemon ` +
    `${describeMismatchBuild(details.daemon, details.cli)}, this CLI ` +
    `${describeMismatchBuild(details.cli, details.daemon)}) and refuses this CLI.`
  );
}
...
export function daemonBuildMismatchSuggestions(details: DaemonBuildMismatchDetails) {
  const attached = details.attachedSessions
    ? `${details.attachedSessions.count} attached window${details.attachedSessions.count === 1 ? "" : "s"}`
    : "an unknown number of attached windows";
  if (details.recommendedAction === "use-newer-hunk") {
    return [
      `Drive sessions with the newer Hunk build the daemon was started from, or run \`hunk daemon restart\` from this build (${attached} would be disconnected and could not reconnect).`,
    ];
  }
  return [
    `Run \`hunk daemon restart\` to replace it, then re-run \`hunk session list\`; windows that could not register attach automatically.`,
    `Restarting disconnects ${attached}; windows on the old build cannot reconnect and must be relaunched (their in-window notes are lost).`,
  ];
}
